	...
	for(;;)
	{
		...
		if(strcmp(command, "exit") == 0)
		{
			...
			else
			{
				...
				do
				{
					printf("clienIP to search is: %d\n", clientIP);
					printf("port to search is: %d\n", ntohs(cport));
					printf("clienIP of curr next at start of do is: %d\n", curr -> fl_next -> clientIP);
					printf("portnum of curr next at start of do is: %d\n", ntohs(curr -> fl_next -> portnum));
					dltptr = curr -> fl_next;
					...
					printf("clienIP of curr next at end of do is: %d\n", curr -> fl_next -> clientIP);
					printf("portnum of curr next at end of do is: %d\n", ntohs(curr -> fl_next -> portnum));
				} while(curr -> fl_next -> clientIP == clientIP && curr -> fl_next -> portnum == cport);
			}
			...
			if(head == NULL) {printf("head is null\n");
				...
				printf("head filename is: %s, portNum is: %d\n", head -> filename, ntohs(head -> portnum));
			}
			pthread_mutex_unlock(&llock);
